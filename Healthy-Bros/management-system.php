...
<div class="main">
  ...
  <div id="users" class="Head" data-filter="users">
    ...
    <a href="admin-signup.php">Add an administrator</a>
    ...
    while ($row = mysqli_fetch_array($query)) {
      echo'<div class="main-container">';
        echo'<div class="user">';
          ...
          { 
            ...
            while($row_posts = $result_posts->fetch_assoc()){
              echo'<h3>';
              echo'<a class="issue_links">';
              if ($row['id']==$row_posts['user_id'] ){
                $counter_post +=1;
                $counter_view += $row_posts['views'];
                $name_posts=$row_posts['title'];
                $myfile = fopen("issues/$name_posts.txt", "r") or die("Unable to open file!");
                echo'<p>';
                  echo fread($myfile,filesize("issues/$name_posts.txt")).str_repeat('&nbsp;', 5).$row_posts['views'].'VIEWS' ;
                echo'</p>';
                fclose($myfile);
              }
            } 
        ...
        echo'<div class="post_view_flex">';
          echo'<h2>';
            echo $counter_post.'POSTS';
          echo'</h2>';
          echo'<h2>';
            echo $counter_view.'Total VIEWS';
          echo'</h2>';  
        echo'</div>';
      echo'</div> ';
